...
export const useSocketStore = create((set) => ({
  ...
  connectSocket: (userId) => {
    ...
    if (!userId) {
      console.warn("Không có userId để kết nối socket");
      return;
    }
    const socket = io(import.meta.env.VITE_SOCKET_URL || "http://localhost:3000", {
      withCredentials: true, // Gửi cookie jwt
      transports: ["websocket"],
      query: { userId } // Truyền userId qua query
    });
    ...
    socket.on("connect_error", (err) => {
      ...
      // Thử reconnect sau 1 giây nếu user vẫn đăng nhập
      setTimeout(() => {
        ...
        const tempMessageIndex = messages.findIndex((msg) => {
          ...
          if (updatedMessage.image && msg.image) {
            return (
              ...
              Math.abs(
                new Date(msg.createdAt) - new Date(updatedMessage.createdAt)
              ) < 10000
            );
          }
    ...
    socket.on("group_member_added", (data) => {
      ...
      if (!data || !data.chatId) {
        console.error(
          "Dữ liệu không hợp lệ từ sự kiện group_member_added:",
          data
        );
        return;
      }
      ...
      if (data.chat) {
        ...
        if (isCurrentUserInGroup) {
            ...
            if (data.userId === currentUserId) {
              toast.success(
                `Bạn đã được thêm vào nhóm "${
                  finalChat.groupName || finalChat.name
                }"`
              );
            } else {
              toast.success(
                `${data.userName || "Thành viên mới"} đã được thêm vào nhóm`
              );
            }
          }
          ...
          useChatStore.setState({
            chats: updatedChats,
            selectedChat:
              selectedChat?.chatId === data.chatId ? finalChat : selectedChat,
          });
        }
      } else {
    ...
    socket.on("group_member_removed", async (data) => {
      ...
      if (!data || !data.chatId) {
        console.error(
          "Dữ liệu không hợp lệ từ sự kiện group_member_removed:",
          data
        );
        return;
      }
      ...
      useChatStore.setState({
        chats: updatedChats,
        selectedChat:
          selectedChat?.chatId === data.chatId ? updatedChat : selectedChat,
      });
    });
    ...
    socket.on("removed_from_group", (data) => {
      ...
      if (!data || !data.chatId) {
        console.error(
          "Dữ liệu không hợp lệ từ sự kiện removed_from_group:",
          data
        );
        return;
      }
      ...
      useChatStore.setState({
        chats: updatedChats,
        selectedChat:
          selectedChat?.chatId === data.chatId ? null : selectedChat,
      });
    ...
    socket.on("admin_assigned", (data) => {
      ...
      const validChats = chats.filter(
        (chat) => chat !== undefined && chat !== null
      );

      // Tìm chat cần cập nhật
      const chatToUpdate = validChats.find(
        (chat) => chat.chatId === data.chatId
      );
      if (!chatToUpdate) {
        console.error("Không tìm thấy chat với ID:", data.chatId);
    ...
    socket.on("group_avatar_updated", (data) => {
      ...
      if (!data || !data.chatId || !data.avatar) {
        console.error(
          "Dữ liệu không hợp lệ từ sự kiện group_avatar_updated:",
          data
        );
        return;
      }
    ...
    socket.on("member_left_group", (data) => {
      ...
      if (!data || !data.chatId) {
        console.error(
          "Dữ liệu không hợp lệ từ sự kiện member_left_group:",
          data
        );
        return;
      }
    ...
    socket.on("new_group_created", (data) => {
      ...
      const existingChat = chats.find(
        (chat) =>
          ...
          (data.chat && chat.chatId === data.chat.chatId)
      );
      ...
      if (!existingChat && data.chat) {
        ...
        // Hiển thị thông báo
        toast.success(
          `Bạn đã được thêm vào nhóm "${newChat.groupName || "Nhóm chat mới"}"`
        );
      } else {
    ...
    socket.on("group_ownership_transferred", (data) => {
      ...
      if (!data || !data.chatId) {
        console.error(
          "Dữ liệu không hợp lệ từ sự kiện group_ownership_transferred:",
          data
        );
        return;
      }
      ...
      if (data.chat) {
        ...
        if (data.newCreatorId === currentUserId) {
          ...
          toast.info(
            `Bạn đã chuyển quyền trưởng nhóm "${groupName}" cho ${newCreatorName}`
          );
        }
      }
    });
    ...
    socket.on("message_unpinned", (data) => {
      ...
      const chatStore = useChatStore.getState();
      const { selectedChat } = chatStore; // **QUAN TRỌNG: Chỉ xử lý nếu đang ở đúng chat**

      if (selectedChat && selectedChat.chatId === chatId) {
        // Gọi fetchPinnedMessages TRƯỚC
        chatStore.fetchPinnedMessages(chatId).then(() => {
          console.log("✅ Đã fetch lại pinnedMessages cho chatId:", chatId); // Sau đó mới cập nhật trạng thái local
